...
try {
    $target = [IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA 'Programs\JA_DUT_Info')).TrimEnd('\')
    $origin = [IO.Path]::GetFullPath($env:UNINSTALL_ORIGIN).TrimEnd('\')
    $key = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\JA_DUT_Info'
    ...
    $purge = $false
    if (-not $silent) {
        if (-not (Confirm-Yes 'Uninstall JA DUT Info? (Yes/No)')) {
            Write-Host 'Uninstall cancelled.'
            exit 0
        }
        $purge = Confirm-Yes 'Delete configurations and logs too? (Yes/No; default No)'
    }
    $exe = Join-Path $target 'ja_dut_info.exe'
    $running = @(Get-Process ja_dut_info -ErrorAction SilentlyContinue | Where-Object { $_.Path -eq $exe })
    if ($running.Count) {
    ...
    $dataDirs = @((Join-Path $env:APPDATA 'JA_DUT_Info'), (Join-Path $env:LOCALAPPDATA 'JA_DUT_Info'))
    if ($purge) {
    ...
    $desktop = Join-Path ([Environment]::GetFolderPath('Desktop')) 'JA DUT Info.lnk'
    $menu = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\JA DUT Info'
    $shell = New-Object -ComObject WScript.Shell
    foreach ($link in @($desktop, (Join-Path $menu 'JA DUT Info.lnk'), (Join-Path $menu 'Uninstall JA DUT Info.lnk'))) {
    ...
    foreach ($name in @('JA DUT Info', 'JA_DUT_Info')) {
