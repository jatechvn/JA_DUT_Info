...
class SrfService {
  ...
  Future<bool> pingMcu(String serial, String serviceName) async {
    try {
      final out = await runCmd(['adb', '-s', serial, 'shell', 'service', 'call', serviceName, '50']);
      final ok = out.contains('00000001');
      logger.info('[SrfService] Ping $serviceName on $serial: ok=$ok ($out)');
      return ok;
    } catch (e) {
      logger.severe('[SrfService] Exception pinging $serviceName on $serial: $e');
      return false;
    }
  }
  ...
  Future<bool> triggerGoldenTransmit(String goldenSerial, String serviceName) async {
    try {
      ...
      logger.info('[SrfService] Golden Panel transmit $serviceName: ok=$ok ($out)');
      return ok;
    } catch (e) {
      logger.severe('[SrfService] Exception triggering Golden Panel transmit: $e');
      return false;
    }
  }
  ...
  Future<SrfResult> verifyDut(String dutSerial, {String? goldenSerial}) async {
    logger.info('[SrfService] Starting verification on DUT: $dutSerial (Golden: $goldenSerial)');

    // 1. Read matrix property
    final matrix = (await runCmd(['adb', '-s', dutSerial, 'shell', 'getprop', 'qolsys.srfslot.matrix'])).trim();
    if (matrix.isEmpty || matrix == '0000' || matrix == 'N/A') {
      logger.info('[SrfService] No SRF card in matrix on DUT $dutSerial');
    ...
    for (final key in propKeys) {
      props[key] = (await runCmd(['adb', '-s', dutSerial, 'shell', 'getprop', key])).trim();
    }
    ...
    // 3. Ping MCU for each slot
    for (final slot in slots) {
      final alive = await pingMcu(dutSerial, slot.serviceName);
      if (!alive) {
        return SrfResult(
