...
class OtaUpdateConfig {
  ...
  factory OtaUpdateConfig.defaults() => const OtaUpdateConfig(
    serverPath: r'\\10.81.141.226\temp\FBT\JA_PROJECT\JA_Update\JA_DUT_Info',
    // Empty credentials let Windows use the signed-in user's SMB session.
    // Explicit credentials can still be entered in the OTA settings dialog.
    username: '',
    password: '',
    checkInterval: 'daily',
    autoDownload: false,
...
class OtaUpdateService extends ChangeNotifier {
  ...
  /// 2. Thư mục AppData (%APPDATA%\JA_DUT_Info\update_config.json)
  File getConfigFile() {
    ...
    if (appData != null && appData.isNotEmpty) {
      final dir = Directory('$appData\\JA_DUT_Info');
      if (!dir.existsSync()) {
        try {
  ...
  Future<void> saveExternalConfigFile(OtaUpdateConfig newConfig) async {
    try {
      _config = newConfig;
      final file = getConfigFile();
      final encoder = const JsonEncoder.withIndent('  ');
      await file.writeAsString(encoder.convert(_config.toJson()), flush: true);
      notifyListeners();
    } catch (e) {
  ...
  Future<UpdateCheckResult> checkForUpdates({
    ...
    // The singleton starts loading external configuration in its constructor.
    // Always wait for that load before either manual or automatic checks so a
    // startup check cannot overwrite or use the default configuration.
    await ready;
    _isChecking = true;
    notifyListeners();
    ...
    try {
      ...
      await saveExternalConfigFile(
        _config.copyWith(lastCheckTime: DateTime.now()),
      );
      ...
      try {
        ...
        // vd: JA_DUT_Info_v2.2.1_Windows_x64.zip -> 2.2.1
        final verRegex = RegExp(
          r'^JA_DUT_Info_[vV]?(\d+\.\d+(?:\.\d+)?(?:-[a-zA-Z0-9.-]+)?(?:\+\d+)?)(?:_[a-zA-Z0-9_]+)?\.zip$',
          caseSensitive: false,
        );
...
""");
    ...
    // Nếu zip đóng gói lồng 1 thư mục gốc (vd: JA_DUT_Info_v2.2.1_Windows_x64)
    final subDirs = extractDir.listSync().whereType<Directory>().toList();
    if (subDirs.length == 1) {
      final testExe = File('${subDirs.first.path}\\ja_dut_info.exe');
      if (testExe.existsSync()) {
        payloadDir = subDirs.first;
      }
    }

    for (final name in ['ja_dut_info.exe', 'flutter_windows.dll', 'data']) {
    ...
    onProgress?.call(1.0, 'Sẵn sàng áp dụng cập nhật! Đang khởi động lại...');
...
chcp 65001 >nul
title JA DUT Info - Dang Cap Nhat Phien Ban Moi...
...
echo ========================================================
echo   JA DUT INFO - DANG TIEN HANH CAP NHAT
echo ========================================================
echo.
...
robocopy "%DST_DIR%" "%BACKUP_DIR%" /E /NP /R:2 /W:1 /XD logs /XF update_config.json config.ini config.json >"%~dp0backup.log"
if errorlevel 8 exit /b 13
robocopy "%SRC_DIR%" "%DST_DIR%" /E /IS /IT /NP /R:5 /W:2 /XD logs /XF update_config.json config.ini config.json >"%~dp0apply.log"
